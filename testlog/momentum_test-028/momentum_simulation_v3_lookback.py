...
Formula: 5×M₁ + 3×Accel₁ + 2×Accel₂ (เพิ่ม M3)
Period: 01-30 Sep 2025 (30 วัน)
Actions: Buy/Sell ONLY (hard-code)
Lookback: ดึงข้อมูลย้อนหลัง 4 ชั่วโมง เพื่อให้ P1-P4 มีค่าจริง
"""
import psycopg2
import pandas as pd
import numpy as np
...
def fetch_trading_data(start_date, end_date, lookback_hours=4):
    """
    ...
    # คำนวน lookback_start_date (ย้อนหลัง X ชั่วโมง)
...
def calculate_momentum_score(pnls):
    """Option D Extended: Acceleration with M3"""
    p1, p2, p3, p4, p5, p6 = pnls
    m1 = p1 - p2  # momentum ล่าสุด
    m2 = p2 - p3  # momentum ก่อนหน้า 1
    m3 = p3 - p4  # momentum ก่อนหน้า 2 (เพิ่มใหม่!)
    
    accel1 = m1 - m2  # acceleration ล่าสุด
    accel2 = m2 - m3  # acceleration ย้อนหลัง (เพิ่มใหม่!)
    
    return (5 * max(m1, 0) + 3 * max(accel1, 0) + 2 * max(accel2, 0))

def run_simulation(start_date, end_date, log_level='info', lookback_hours=4):
...
if __name__ == "__main__":
    ...
    print("="*120)
    print("🚀 Test 028: Acceleration Extended + 4-Hour Lookback")
    ...
    print(f"\n✅ Test 028 Complete!")
    print(f"Formula: Acceleration Extended (5×M₁ + 3×Accel₁ + 2×Accel₂)")
