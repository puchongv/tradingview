...
def fetch_all_strategies(start_date, end_date):
    """Fetch all unique strategy-action combinations from database (DYNAMIC ALL ACTIONS)"""
    ...
    ORDER BY strategy_action;
    """
    df = pd.read_sql_query(query, conn)
    conn.close()
    
    full_strategies = df['strategy_action'].tolist()
    
    return full_strategies

def fetch_trading_data(start_date, end_date):
    """Fetch trading data (ALL ACTIONS - DYNAMIC)"""
    conn = psycopg2.connect(**DB_CONFIG)
    query = f"""
...
def run_simulation(start_date, end_date, log_level='info'):
    ...
    if log_level in ['info', 'debug']:
        print(f"📊 Found {len(full_strategies)} combinations (ALL ACTIONS - DYNAMIC)\n")
...
if __name__ == "__main__":
    ...
    print("="*120)
    print("🚀 Test 019: Acceleration Formula - ALL Actions (Dynamic)")
    print(f"📅 Period: {START_DATE} to {END_DATE} (8-9 วัน)")
    print(f"📊 Log Level: {LOG_LEVEL.upper()}")
    print(f"🎯 Actions: ALL ACTIONS (Dynamic scan)")
