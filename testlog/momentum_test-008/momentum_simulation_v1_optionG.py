...
Version: 1.6 (Option G - Penalty for Negative Momentum)
Date: 2025-10-01

Scoring Formula:
- RecentScore_raw = 5×max(M₁,0) - 2×max(-M₁,0) + 4×max(M₂,0) - 1.5×max(-M₂,0) + 3×max(M₃,0) - 1×max(-M₃,0) + 2×max(M₄,0) + 1×max(M₅,0)
- ลงโทษ strategies ที่ momentum ติดลบ
...
def calculate_momentum_score(pnls):
    ...
    OPTION G: Penalty for Negative Momentum
    ...
    # Momentum score with penalty for negative momentum
    recent_raw = (5 * max(m1, 0) - 2 * max(-m1, 0) +  # Heavy penalty for recent negative momentum
                  4 * max(m2, 0) - 1.5 * max(-m2, 0) + 
                  3 * max(m3, 0) - 1 * max(-m3, 0) + 
                  2 * max(m4, 0) + 
                  1 * max(m5, 0))
...
if __name__ == "__main__":
    ...
    print("="*120)
    print("🚀 Starting Momentum-based Strategy Scoring Simulation (OPTION G: Penalty for Negative Momentum)")
    print(f"📅 Period: {START_DATE} to {END_DATE}")
    print(f"💰 Investment: ${BET_SIZE}/trade, Payout: {PAYOUT}")
    print(f"🔍 Mode: Scan ALL strategies, select TOP {TOP_N} every hour")
    print(f"⚠️  Penalty: -2× for M₁, -1.5× for M₂, -1× for M₃")
    print("="*120)
