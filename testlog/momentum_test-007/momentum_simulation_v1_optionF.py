...
Version: 1.5 (Option F - Hybrid: Momentum + Absolute Performance)
Date: 2025-10-01

Scoring Formula:
- Momentum_Score = 5×max(M₁,0) + 4×max(M₂,0) + 3×max(M₃,0) + 2×max(M₄,0) + 1×max(M₅,0)
...
def calculate_momentum_score(pnls):
    ...
    OPTION F: Hybrid (Momentum + Absolute Performance)
    ...
    # Calculate absolute performance score (normalized)
    absolute_score = p1 / 100  # Normalize by dividing by 100
    
    # Hybrid score: 70% momentum, 30% absolute performance
...
if __name__ == "__main__":
    ...
    print("="*120)
    print("🚀 Starting Momentum-based Strategy Scoring Simulation (OPTION F: Hybrid)")
    print(f"📅 Period: {START_DATE} to {END_DATE}")
    print(f"💰 Investment: ${BET_SIZE}/trade, Payout: {PAYOUT}")
    print(f"🔍 Mode: Scan ALL strategies, select TOP {TOP_N} every hour")
    print(f"⚖️  Formula: 0.7×Momentum + 0.3×Absolute PNL")
    print("="*120)
