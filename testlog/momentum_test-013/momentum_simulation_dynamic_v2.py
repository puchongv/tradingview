...
def calculate_momentum_score(pnls):
    ...
    momentum_score = (5 * max(m1, 0) + 4 * max(m2, 0) + 3 * max(m3, 0) + 2 * max(m4, 0) + 1 * max(m5, 0))
    absolute_score = p1 / 100
    return 0.7 * momentum_score + 0.3 * absolute_score
...
if __name__ == "__main__":
    ...
    print(f"✅ Test 013 Complete! Final PNL: ${r['total_pnl']:.0f}")
