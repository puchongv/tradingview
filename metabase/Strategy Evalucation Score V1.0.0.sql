-- File: Strategy Evalucation Score V1.0.0.sql
...
--   | 24h_max_lose_streak | 24h_pnl | 12h_pnl | 6h_pnl | 3h_pnlate | pnl_score | win_con_scrore | perf_score | total score
...
recent_raw AS (
    SELECT
        aw.*,
        GREATEST(0::numeric, COALESCE(aw.pnl_1h_scoring, 0) - COALESCE(aw.pnl_2h_scoring, 0)) * 5
      + GREATEST(0::numeric, COALESCE(aw.pnl_1h_scoring, 0) - COALESCE(aw.pnl_3h_scoring, 0)) * 4
      + GREATEST(0::numeric, COALESCE(aw.pnl_1h_scoring, 0) - COALESCE(aw.pnl_4h_scoring, 0)) * 3
      + GREATEST(0::numeric, COALESCE(aw.pnl_1h_scoring, 0) - COALESCE(aw.pnl_5h_scoring, 0)) * 2
      + GREATEST(0::numeric, COALESCE(aw.pnl_1h_scoring, 0) - COALESCE(aw.pnl_6h_scoring, 0)) * 1 AS recent_score_raw
    FROM agg_windows aw
),
recent_stats AS (
    SELECT
...
score_totals AS (
    SELECT
        ...
        LEAST(rp.recent_performance_max_score,
              GREATEST(0::numeric, sc.recent_score_raw / NULLIF(sc.recent_kpi, 0) * rp.recent_performance_max_score)) AS perf_score
    FROM score_compiled sc
    CROSS JOIN (SELECT recent_performance_max_score FROM params) rp
),
joined AS (
    SELECT
        st.strategy,
...
SELECT
    ...
    CASE WHEN j.abs_pnl THEN ABS(COALESCE(j.pnl_3h_raw, 0)) ELSE COALESCE(j.pnl_3h_raw, 0) END AS "3h_pnlate",
