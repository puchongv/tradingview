-- File: Strategy Evalucation Score V1.6.sql
-- Purpose: Calculate comprehensive strategy evaluation scores with reverse mode support (FIXED SCORING LOGIC ONLY)
...
-- Version: 1.6 (SAME METABASE FORMAT AS V1.4 + FIXED ACTION MAPPING IN LOGIC ONLY)
...
WITH param_raw AS (
    SELECT
        -- Use EXACT same parameter parsing method as V1.4 (SAME AS V1.0.0)
...
SELECT 
    strategy,
    -- FIXED: Map long action names to short ones for Metabase display (ONLY CHANGE FROM V1.4)
    CASE 
    ...
    -- PNL calculations using parameters (SAME AS V1.4)
    CASE 
    ...
    CASE 
        ...
        END)
    END AS "3h_pnl",
    -- PNL Score (40 points) using parameters (SAME AS V1.4)
    ...
    -- Winrate Score (30 points) using parameters (FIXED: Use 55.56% break-even for binary options)
    ...
    GREATEST(0::numeric, LEAST(pr.winrate_consistency_score, 
        pr.winrate_consistency_score * 0.8)) AS win_con_score30, -- Simplified consistency (SAME AS V1.4)
    -- Performance Score (30 points) using parameters (FIXED: Bidirectional performance change)
    GREATEST(0::numeric, LEAST(pr.recent_performance_max_score,
        -- Baseline = 15, can go up or down based on 6h vs 12h performance
        CASE 
    ...
    GREATEST(0::numeric, LEAST(pr.recent_performance_max_score,
        -- Same bidirectional performance calculation as above
        CASE 
            WHEN SUM(CASE WHEN entry_time >= NOW() - INTERVAL '12 hours' AND result_10min = 'WIN' THEN pr.investment * pr.payout
                           WHEN entry_time >= NOW() - INTERVAL '12 hours' AND result_10min = 'LOSE' THEN -pr.investment ELSE 0 END) = 0 THEN 0
            ELSE (pr.recent_performance_max_score / 2.0) + 
...
-- REVERSE strategies (only when reverse_enable = true) - SAME AS V1.4 WITH ACTION MAPPING
SELECT 
    strategy,
    -- FIXED: Map long action names to short ones for Metabase display (ONLY CHANGE FROM V1.4)
    CASE 
    ...
    0::bigint AS "24h_max_win_streak",
    0::bigint AS "24h_max_lose_streak",
    -- Same PNL calculations with abs_pnl parameter (SAME AS V1.4)
    CASE 
        WHEN pr.abs_pnl = true THEN ABS(SUM(CASE 
            WHEN entry_time >= NOW() - INTERVAL '24 hours' AND result_10min = 'WIN' THEN pr.investment * pr.payout
            WHEN entry_time >= NOW() - INTERVAL '24 hours' AND result_10min = 'LOSE' THEN -pr.investment
    ...
    CASE 
        ...
        END)
    END AS "3h_pnl",
    -- REVERSE PNL Score (40 points) - Lower PNL = Higher Score (SAME AS V1.4)
    GREATEST(0::numeric, LEAST(pr.pnl_score_72h, 
    ...
    GREATEST(0::numeric, LEAST(pr.pnl_score_12h,
        (pr.pnl_kpi * 0.7 - SUM(CASE 
            ...
            ELSE 0 
        END)) / (pr.pnl_kpi * 0.7) * pr.pnl_score_12h)) AS pnl_score40,
    -- REVERSE Winrate Score (30 points) - Lower Winrate = Higher Score (SAME AS V1.4)
    GREATEST(0::numeric, LEAST(pr.winrate_score_72h,
        (45 - CASE 
            WHEN COUNT(*) FILTER (WHERE entry_time >= NOW() - INTERVAL '72 hours' AND result_10min IN ('WIN', 'LOSE')) = 0 THEN 0::numeric
            ELSE 100.0 * COUNT(*) FILTER (WHERE entry_time >= NOW() - INTERVAL '72 hours' AND result_10min = 'WIN')::numeric 
    ...
    -- REVERSE Performance Score (30 points) - Declining = Higher Score (SAME AS V1.4)
    GREATEST(0::numeric, LEAST(pr.recent_performance_max_score,
        (SUM(CASE 
            WHEN entry_time >= NOW() - INTERVAL '12 hours' AND result_10min = 'WIN' THEN pr.investment * pr.payout
            WHEN entry_time >= NOW() - INTERVAL '12 hours' AND result_10min = 'LOSE' THEN -pr.investment
        ...
        END) - SUM(CASE 
            ...
            WHEN entry_time >= NOW() - INTERVAL '6 hours' AND result_10min = 'LOSE' THEN -pr.investment
            ELSE 0 
        END)) / 200.0 * pr.recent_performance_max_score)) AS perf_score30,
    -- Total Score (simplified for REVERSE) (SAME AS V1.4)
