-- File: line-chart winrate overtime_v1_0_0.sql
-- Purpose: Line chart time series of win rate per day, breakout by strategy | action.
--          Configure chart as: Y = y_date (time), X = x_win_rate_pct, Series = series.
-- Version: 1.0.0 (new file)

-- Metabase params (same set as barchart v1_0_3; all optionals keep behavior):
...
--   {{payout}} (Number, required, default 0.8)       -- not used in winrate; kept for parity
--   {{investment}} (Number, required, default 250)   -- not used in winrate; kept for parity
