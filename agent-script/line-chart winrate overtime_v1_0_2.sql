-- File: line-chart winrate overtime_v1_0_2.sql
-- Purpose: Hourly win rate per strategy | action with change vs previous hour.
-- Chart config: Y = y_ts (time), X = x_win_rate_pct or delta_win_rate_pct, Series = series.
-- Version: 1.0.2 (new file)

-- Params (same as v1_0_1):
...
deltas AS (
    SELECT
        ...
        series,
        x_win_rate_pct,
        LAG(x_win_rate_pct) OVER (PARTITION BY series ORDER BY y_ts)    AS prev_win_rate_pct,
        ROUND((x_win_rate_pct - LAG(x_win_rate_pct) OVER (PARTITION BY series ORDER BY y_ts)), 1) AS delta_win_rate_pct,
        ROUND(
            CASE WHEN LAG(x_win_rate_pct) OVER (PARTITION BY series ORDER BY y_ts) = 0 THEN NULL
                 ELSE 100.0 * (x_win_rate_pct - LAG(x_win_rate_pct) OVER (PARTITION BY series ORDER BY y_ts))
                      / NULLIF(LAG(x_win_rate_pct) OVER (PARTITION BY series ORDER BY y_ts), 0)
            END, 1
