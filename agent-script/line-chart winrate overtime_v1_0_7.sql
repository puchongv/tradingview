-- File: line-chart winrate overtime_v1_0_7.sql
...
--   {{past_hours}} (Number, optional)      -> look-back window in hours relative to latest timestamp
...
date_bounds AS (
    SELECT MAX(entry_time) AS max_entry_time FROM filtered
),
filtered_recent AS (
    SELECT f.*
    FROM filtered f
    ...
    WHERE 1=1
    [[ AND f.entry_time >= b.max_entry_time - (CAST({{past_hours}} AS int) * INTERVAL '1 hour') ]]
),
series_overall AS (
    SELECT
        strategy,
