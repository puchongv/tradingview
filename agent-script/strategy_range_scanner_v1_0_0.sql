/* Strategy Range Scanner - V1.0.0 */

/*
 Scan signals over a date range (Time Bucket → Signal)
   Purpose:
     ...
     - Filter by user-defined thresholds and a concrete date range

   Params (Metabase variables):
     timeframes_csv            : '10,30,60' (Text; accepts 10|10min|30|30min|60|60min)
     start_date                : 'YYYY-MM-DD' (Text; inclusive)
     end_date                  : 'YYYY-MM-DD' (Text; inclusive)
...
WITH params AS (
  SELECT
    -- Date range params
    (CASE WHEN {{start_date}} = '' THEN NULL ELSE {{start_date}}::timestamp END) AS start_ts,
    (CASE WHEN {{end_date}}   = '' THEN NULL ELSE {{end_date}}::timestamp   END) AS end_ts,
    -- Other params
...
base AS (
  ...
  WHERE EXTRACT(DOW FROM t.entry_time)::int = b.dow
    ...
    AND (
      (p.start_ts IS NULL AND p.end_ts IS NULL)
      OR (
        (p.start_ts IS NULL OR t.entry_time >= p.start_ts)
        AND (p.end_ts IS NULL OR t.entry_time < p.end_ts + interval '1 day')
      )
    )
...
components AS (
  SELECT s.timeframe, s.dow, s.hr, s.strategy_action,
         ...
         p.investment, p.payout, p.quality_mode,
...
WHERE c.total_trades >= p.min_trades
  AND (
    (p.min_signal_quality IS NULL) OR (
      CASE WHEN c.quality_mode='practical' THEN 10.0 * (0.70*exp_norm + 0.20*LEAST(1.0, GREATEST(0.0, (rel_margin + 10.0)/10.0)) + 0.10*sample21)
           WHEN c.quality_mode='strict'    THEN (CASE WHEN rel_margin < 0 THEN LEAST(5.0, 10.0 * (0.60*exp_norm + 0.25*LEAST(1.0, GREATEST(0.0, (rel_margin + 10.0)/10.0)) + 0.15*sample51))
                                                     ELSE 10.0 * (0.60*exp_norm + 0.25*LEAST(1.0, GREATEST(0.0, (rel_margin + 10.0)/10.0)) + 0.15*sample51) END)
           ELSE LEAST(10.0, 10.0 * (0.60*exp_norm + 0.25*LEAST(1.0, GREATEST(0.0, (rel_margin + 20.0)/30.0)) + 0.15*sample21))
      END >= p.min_signal_quality
    )
  )
  AND (
    (p.min_stability IS NULL) OR (
      CASE
        WHEN c.quality_mode='practical' THEN LEAST(10.0, GREATEST(1.0, 10.0 - (COALESCE(c.stddev_pct, 30.0) / 3.0)))
        WHEN c.quality_mode='strict'    THEN LEAST(10.0, GREATEST(1.0, 10.0 - (COALESCE(c.stddev_pct, 30.0) / 2.5)))
        ELSE LEAST(10.0, GREATEST(1.0, 10.0 - (COALESCE(c.stddev_pct, 30.0) / 2.8)))
